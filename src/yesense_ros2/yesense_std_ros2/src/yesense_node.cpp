...
class YESENSE_Publisher : public rclcpp::Node
{
	...
	void timer_callback()
	{
		...
		{	
			...
			{
				size_t read_len = std::min(available_bytes, static_cast<size_t>(UART_RX_BUF_LEN));
				bytes_read_r_buffer = ser.read(r_buffer, read_len);
			}
		}
		else if(serial_drv_linux == driver_type)
		{
			ssize_t nread = read(fd, r_buffer, UART_RX_BUF_LEN);
			if(nread <= 0)
			{
				return;
			}
			bytes_read_r_buffer = static_cast<size_t>(nread);
		}	

		if(bytes_read_r_buffer == 0u)
		{
			return;
		}
